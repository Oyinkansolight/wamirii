...
export default AuthGuardHOC(() => {
  ...
  const c = useMemo(() => {
    const constraints: QueryConstraint[][] = [
      FirestoreService.getListingsConstraints({ hasFollowUp: true }),
      FirestoreService.getListingsConstraints({ hasFollowUp: true }),
    ];
  ...
  useEffect(() => {
    if (idx === 1) {
      setSortByField({ fieldName: 'followUpUpdatedAt', direction: 'desc' });
    } else {
    ...
    <DashboardLayout2>
      <div className='layout flex h-screen flex-col gap-6'>
        <div className='flex flex-col items-start justify-between gap-4 lg:flex-row'>
          <div>
            <div className='text-3xl font-extrabold'>Follow Ups</div>
            <div className='font-light text-[#819289]'>
              Stay up to date with follow ups made on the platform
            </div>
          </div>
        ...
        <TabBar
          ...
          items={[
            { label: `Follow Ups (${mySubmissionsCount})` },
            { label: `New Follow Ups (${allSubmissionsCount})` },
          ]}
        />
        {error && (
          <div
